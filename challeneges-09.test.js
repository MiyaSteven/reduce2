...
const countNumberOfElements = (arr) => {
    const totalElements = (sum, total) => sum + (total/total);
    return arr.reduce(totalElements);
};
...
const returnNames = (arr) => {
    let movies = [];
    const characterNames = (a, b) => {
        movies.push(b.name);
        return a;
    }
    arr.reduce(characterNames, 0);
    return movies;
};
